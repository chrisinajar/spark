...
#undef P
    ...
    NO_INLINE void sGetBendInfo(CRenderObject* const __restrict renderObject, FrameType frameType, UFloat4* sData, CD3D9Renderer* r)
    {
    ...
    NO_INLINE void sTextureTileSize(UFloat4* sData, SRenderPipeline& rRP)
    {
        ...
        if (pOD && pOD->m_pParticleParams)
        {
            ...
            ParticleParams::STextureTiling const& tiling = pOD->m_pParticleParams->TextureTiling;
    ...
    NO_INLINE void sParticleParams(UFloat4* sData, SRenderPipeline& rRP)
    {
        ...
        if (pOD && pOD->m_pParticleParams)
        {
            // xy = xy expansion (-1) / and curving (+1), z = curvature amount, w = backlighting
            ParticleParams const& params = *pOD->m_pParticleParams;
            ParticleParams::EFacing eFacing = params.eFacing;
            sData[0].f[0] = eFacing == eFacing.Camera || eFacing == eFacing.CameraX ? 1.f       // X expansion and curvature
                : -1.f;                                                                                                                                             // X expansion only
            sData[0].f[1] = params.HasVariableVertexCount() ? 0.f                                                       // no Y expansion or curvature
                : eFacing == eFacing.Camera ? 1.f                                                                                           // Y expansion and curvature
                : -1.f;                                                                                                                                             // Y expansion only
            sData[0].f[2] = eFacing == eFacing.Camera || eFacing == eFacing.CameraX ? +params.fCurvature
                : 0.f;
            sData[0].f[3] = params.fDiffuseBacklighting;
        }
    }
    ...
    NO_INLINE void sParticleSoftParams(UFloat4* sData, SRenderPipeline& rRP)
    {
        ...
        if (pOD && pOD->m_pParticleParams)
        {
            const ParticleParams& params = *pOD->m_pParticleParams;
            float softness = params.bSoftParticle.fSoftness;
            sData[0].f[0] = powf(32.0f, -softness + 1.0f);

            // rescale spherical approximation value slightly, a value of 1.0f can cause the particle vertices
            // to lie on the camera plane and thus result in nan values in the interpolated values (hpos.w==0)
            sData[0].f[1] = params.fSphericalApproximation * 0.999f;

            sData[0].f[2] = params.fVolumeThickness;
        }
    }

    NO_INLINE void sParticleParams3(UFloat4* sData, SRenderPipeline& rRP)
    {
        SRenderObjData* pOD = rRP.m_pCurObject->GetObjData();
        if (pOD && pOD->m_pParticleParams)
        {
            const ParticleParams& params = *pOD->m_pParticleParams;
            sData[0].f[0] = params.GetTexcoordVMultiplier();
            sData[0].f[1] = params.fEnvProbeLighting;
        }
    }
    ...
    NO_INLINE void sParticleAlphaTest(UFloat4* sData, SRenderPipeline& rRP)
    {
        ...
        if (pOD && pOD->m_pParticleParams)
        {
            ...
            // lod blending information passed via m_fTempVars
            float fAlphaScale = pOD->m_pParticleParams->fAlpha.GetMaxBaseLerp() * pOD->m_fTempVars[0];
            ParticleParams::SAlphaClip const& clip = pOD->m_pParticleParams->AlphaClip;
            sData[0].f[0] = clip.fScale.Min * fAlphaScale;
            sData[1].f[0] = (clip.fScale.Max - clip.fScale.Min) * fAlphaScale;
            sData[0].f[1] = clip.fSourceMin.Min;
            sData[1].f[1] = clip.fSourceMin.Max - clip.fSourceMin.Min;
            sData[0].f[2] = clip.fSourceWidth.Min;
            sData[1].f[2] = clip.fSourceWidth.Max - clip.fSourceWidth.Min;
        }
    }
        ...
        void* outputData)
    {
        ...
        for (AZ::u32 parameterIdx = 0; parameterIdx < parameterCount; parameterIdx++)
        {
            ...
            for (AZ::u32 componentIndex = 0; componentIndex < 4; componentIndex++)
            {
                ...
                //spark stuff
                case ECGP_PB_FogOfWar:
                {
                    auto console = gEnv->pConsole;
                    ICVar* var = console->GetCVar("g_FogOfWar");

                    if(var && var->GetIVal() > 0){
                        auto GetFloatOr=[](ICVar* var,float default_value){ return var ? var->GetFVal() : default_value;};

                        result[0].f[0] = GetFloatOr(console->GetCVar("g_FogOfWar_min_x"),0.0f);
                        result[0].f[1] = GetFloatOr(console->GetCVar("g_FogOfWar_min_y"),0.0f);
                        result[0].f[2] = GetFloatOr(console->GetCVar("g_FogOfWar_max_x"),0.0f);
                        result[0].f[3] = GetFloatOr(console->GetCVar("g_FogOfWar_max_y"),0.0f);
                    }
                    else
                    {
                        result[0].f[0] = 0.0f;
    ...
    void* outputData)
{
    ...
    for (AZ::u32 parameterIdx = 0; parameterIdx < parameterCount; parameterIdx++)
    {
        ...
        case ECGP_PI_ParticleParams:
            sParticleParams(result, rRP);
            break;
        case ECGP_PI_ParticleSoftParams:
            sParticleSoftParams(result, rRP);
            break;
        case ECGP_PI_ParticleExtParams:
            sParticleParams3(result, rRP);
            break;
        case ECGP_PI_ParticleAlphaTest:
            sParticleAlphaTest(result, rRP);
            break;
...
bool CHWShader_D3D::mfSetTextures(const std::vector<SCGTexture>& Textures, EHWShaderClass eSHClass)
{
    ...
    for (i = 0; i < nSize; i++)
    {
        ...
        //spark stuff
        case ECGT_FogOfWarMap:
        {
            CTexture* tex = static_cast<CTexture*>(gEnv->pRenderer->EF_GetTextureByName("FogOfWarTex"));
            if(tex)
            {
                tex->ApplyTexture(nTUnit, eSHClass, nResViewKey);
            }
...
void SBending::GetShaderConstantsStatic(float realTime, Vec4* pBendInfo) const
{
...
void CHWShader_D3D::mfSetForOverdraw(SHWSInstance* pInst, uint32 nFlags, uint64& RTMask)
{
    ...
    if (CRenderer::CV_r_measureoverdraw == 2 || CRenderer::CV_r_measureoverdraw == 4)
    {
